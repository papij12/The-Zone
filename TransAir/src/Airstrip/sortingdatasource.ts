import { abstractsortingDataSource } from "./abstractsortingDataSource";
...
export class SortingDataSource extends abstractsortingDataSource {

 protected loadCards(): Promise<Card[]> {
     return Axios.get(urls.cards).then(response => response.data);
 }

 storesortingCard(): Promise<number> {
